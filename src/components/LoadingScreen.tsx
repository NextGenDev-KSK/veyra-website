...
import { LoadingLogoReveal } from "./LoadingLogoReveal";
...
export function LoadingScreen({ onStarted }: LoadingScreenProps) {
  ...
  const handleBegin = () => {
    setFadeOut(true);
    // Start background audio + logo reveal
    window.dispatchEvent(new CustomEvent("start-background-audio"));
    window.dispatchEvent(new CustomEvent("veyra-experience-start"));
      ...
      <div className="relative flex flex-col items-center">
        ...
        <div className="relative z-10 flex flex-col items-center gap-10">
          <div className="flex flex-col items-center gap-6">
            <LoadingLogoReveal />
            <h1 className="font-display text-2xl md:text-4xl font-black uppercase tracking-[0.4em] text-white text-center">
              VEYRA SOUNDS
            </h1>
            <div className="h-[1px] w-48 bg-gradient-to-r from-transparent via-[#8b5cf6]/60 to-transparent" />
          </div>
          ...
          {!isReady ? (
            <div className="flex flex-col items-center gap-6">
              <div className="relative h-[2px] w-80 overflow-hidden rounded-full bg-white/10">
                <div
                  className="absolute left-0 h-full bg-white transition-all duration-700 ease-out shadow-[0_0_20px_rgba(167,139,250,0.8)]"
                  style={{ width: `${visualProgress}%` }}
                />
              </div>
              <div className="flex w-full items-center justify-between text-[11px] font-bold uppercase tracking-[0.4em] text-white/30">
                <span className="animate-pulse">Initializing Audio Engine</span>
            ...
            >
              <div className="absolute inset-0 border border-white/20 rounded-full group-hover:border-[#8b5cf6]/70 transition-colors" />
